...
save_folder = "/home/hk-project-pai00005/xo8179/neural_lam_fork/neural-lam/eval_ukesm/rmse_r2_final/values_interpolated/"
...
def compute_r2_for_variable(
    ...
    df_r2.to_csv(
        save_folder
        + f"r2_all_variables_{current_model_resolution}_{'past' if past else 'future'}{var_start if var_start > 0 else ''}_interpolated.csv"
    )
    ...
    map_save = (
        save_folder
        + f"r2_maps_{current_model_resolution}_{'past' if past else 'future'}_interpolated.zarr"
    )
...
def compute_rmse_for_variable(
    ...
    save_path = (
        save_folder
        + f"rmse_all_variables_{current_model_resolution}_{'past' if past else 'future'}{var_start if var_start > 0 else ''}_interpolated.csv"
    )
    ...
    map_save = (
        save_folder
        + f"rmse_maps_{current_model_resolution}_{'past' if past else 'future'}{var_start if var_start > 0 else ''}_interpolated.zarr"
    )
...
def main():
    ...
    selected_model_past = plot.PAST_MODELS_UKESM_HOLES_FILLED[model_idx]
    selected_model_future = plot.FUTURE_MODELS_UKESM_HOLES_FILLED[model_idx]
