...
models_temp = utils.PAST_MODELS  # + future_models
future_models_temp = utils.FUTURE_MODELS
...
for model in models:
    ...
    for run in [1, 2, 3]:
        try:
            ...
            model["mae"].append(
                pd.read_csv(
                    model["folder_long"][run - 1] + "/files/test_mae.csv", header=None
                )
            )
        try:
            model["rmse"].append(
                pd.read_csv(
                    model["folder_long"][run - 1] + "/test_rmse.csv", header=None
                )
            )
        except Exception:
            model["rmse"].append(
                pd.read_csv(
                    model["folder_long"][run - 1] + "/files/test_rmse.csv", header=None
                )
            )
