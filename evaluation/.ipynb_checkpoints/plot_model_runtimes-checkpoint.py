...
models = [x for x in util.PAST_MODELS_UKESM if x["train_time"] is not None] if plot_ukesm else [x for x in util.PAST_MODELS_NG_era5 if x["train_time"] is not None]

model_short_names=['UKESM z64-MM','UKESM z128-MM','UKESM z256-MM','UKESM z64-H', 'Persistence']if plot_ukesm else  ['NG z64-MM','NG z128-MM','NG z256-MM','NG z64-H', 'Era5 z64-MM']
...
colors = [f'C{i}' for i in range(len(models))] if plot_ukesm else [f'C{i}' for i in range(len(models)-1)] + [f'C{len(models)}']
...
ax.grid()
ax.set_title("Training Time for Models Trained on UKESM per Step" if plot_ukesm else "Training Time for Models Trained on NextGEMS or ERA5 per Step")
ax.legend(loc='upper center')

plt.tight_layout()
plt.savefig(f"{plot_folder}/model_compute_times_UKESM.pdf" if plot_ukesm else f"{plot_folder}/model_compute_times_NG.pdf", dpi=400)
