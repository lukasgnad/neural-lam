...
models = [x for x in util.PAST_MODELS if x["train_time"] is not None]
...
ax.set_xticklabels(
    [
        "Step 1 (70 epochs, 1 ahead)",
        "Step 2 (20 epochs, 4 ahead)",
        "Step 3 (20 epochs, 8 ahead)",
    ]
)
ax.set_ylabel("Time in hours")
ax.grid()
ax.set_title("Training time per step per model")
ax.legend()

plt.tight_layout()
plt.savefig(f"{plot_folder}/model_compute_times.png", dpi=400)
