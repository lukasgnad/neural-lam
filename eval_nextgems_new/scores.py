...
save_folder = "/home/hk-project-pai00005/xo8179/neural_lam_fork/neural-lam/eval_nextgems_new/values_final/"
...
def get_true_data_matching_forecast_timestamps(
    ...
    for t in range(len(new_forecast.time.values) - 1):
        if (
            not new_forecast.time.values[t + 1] - new_forecast.time.values[t]
            == 43200000000000
        ):
            print(
                f"Error with {new_forecast.time.values[t]} and {new_forecast.time.values[t+1]}: difference is {str(new_forecast.time.values[t+1] - new_forecast.time.values[t])}"
            )
...
def get_stacked_true_data_matching_forecast_timestamps(ds_truth, ds_forecast):
    ...
    new_truth = new_truth.assign_coords(
        {"prediction_timedelta": new_forecast["prediction_timedelta"]}
    )
    return new_truth, new_forecast
...
def compute_r2_for_variable(
    ...
    ds_r2_maps["prediction_timedelta"].attrs.pop("units", None)
...
def compute_rmse_for_variable(
    ...
    ds_rmse_maps["prediction_timedelta"].attrs.pop("units", None)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Model type (0=z64 era5, 1=z64, 2=z128, 3=z256, 4=z64 hierarchical)",
    )
    parser.add_argument(
        "--past",
        type=int,
    ...
    ng_past_forecasts = xr.open_zarr(
        selected_model_past["forecasts"], decode_timedelta=True
    ).astype(np.float32)
    ng_future_forecasts = xr.open_zarr(
        selected_model_future["forecasts"], decode_timedelta=True
    ).astype(np.float32)

    ng_past_truth = xr.open_zarr(selected_model_past["ground_truth"]).astype(
        np.float32
    )
    ng_future_truth = xr.open_zarr(
    ...
    for key in range(var_start, len(plot.KEY_VARIABLES)):
