# Usage Advice (Lukas Gnad)

## Preprocessing the data

To train and test models on global weather data, the data must first be regridded on the desired temporal and spatial resolution. The regridding of spatial resolution can be done through code in `regrid_weatherbench/weatherbench2`, which is the code used for regridding of ERA5 data, adapted to NextGEMS and UKESM data.

Once the desired resolution is reached, the dataset has to be prepared using 4 scripts (also described below):

* `create_global_mesh.py`, to create the mesh graphs
* ~~`create_global_forcing.py`~~ -> `create_global_forcing_stable.py`, to create the forcing (stable means code is adapted to 360 day years of UKESM data)
...
Make sure to create the parameter weights only on training data. If the test data is stored in a different directory, copy the parameter weights folder from the training data to the test data.

## Running the model

The model can be trained or tested through `train_model.py`. This code has been adapted to support datasets differing from ERA5 (e.g. NextGEMS, UKESM), the dataset type can be passed as job argument.
To support other datasets, write a new config under `neural_lam/configs` by extending `base.py`.
